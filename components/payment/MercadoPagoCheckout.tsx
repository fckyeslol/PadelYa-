...
// Minimal types for the MP Bricks vanilla SDK
declare global {
  interface Window {
    MercadoPago?: new (
      publicKey: string,
...
export function MercadoPagoCheckout({ matchId, orgFeeCop, autoStart = false }: Props) {
  ...
  // Auto-start checkout when redirected after publishing a match
  useEffect(() => {
  ...
  // Reload after successful inline payment
  useEffect(() => {
  ...
  // Mount the Payment Brick once script + preferenceId are ready
  useEffect(() => {
    if (step !== "paying" || !scriptLoaded || !preferenceId || !window.MercadoPago) return;
  ...
  if (step === "paying") {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        ...
        />
        {/* MP injects the form into this div */}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
        ...
        >
          ...
          >
            ...
            <span style={{ color: "var(--primary)" }}>{fmt(totalCop)}</span>
          </div>
        </div>
